...
static int axi_xadc_dma_init()
{
    ...
    axi_xadc_dev->rx_chan= dma_request_chan(&axi_xadc_dev->pdev->dev, "axidma1");
    
    if (IS_ERR(axi_xadc_dev->rx_chan))
	{
		dev_err(&axi_xadc_dev->pdev->dev, "No DMA Rx channel\n");
		dma_release_channel(axi_xadc_dev->rx_chan);
		return status;
	}
	if (axi_xadc_dev->rx_chan == NULL)
	{
		dev_err(&axi_xadc_dev->pdev->dev, "No DMA Rx channel\n");
		kfree(axi_xadc_dev);
		return status;
...
static int axi_xadc_dma_config()
{
    ...
    set_user_nice(current, 10); // Need to monitor here THIHA
    axi_xadc_dev->flags =
...
static void axi_xadc_dma_stop(struct dma_chan *chan)
{
	...
	chan_dev->device_terminate_all(axi_xadc_dev->rx_chan);
    }
}
			       ...
			       unsigned int cmd, unsigned long arg)
{
    ...
    //struct xilinx_dma_config config; //THIHA NO MORE
    struct dma_device *rx_dev = NULL;
	...
	/* Only one interrupt */
	//config.coalesc = 1; //THIHA NO MORE
	//config.delay = 0;  //THIHA NO MORE
